...
const collections = [
  ...
  { id: 'athlete', name: 'Athlete Collection', color: '#1A2744', icon: '⚡', tagline: 'Motivation centered on discipline, preparation, and resilience.', desc: 'For competitors, coaches, and anyone who understands that the work is never finished.', messages: ['Never Finished.', 'Earn Tomorrow.', 'Outwork Yesterday.', 'Built Through Adversity.', 'Strong Mind. Strong Finish.'], products: 'Performance tees · Training gear · Shorts' },
      ...
      <section style={{ background: 'var(--cream)', padding: '80px 0' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 48 }}>
            {collections.map(c => (
              <button key={c.id} onClick={() => setActive(c.id)} style={{
                ...
                background: active === c.id ? c.color : 'var(--warm-white)',
                color: active === c.id ? 'white' : 'var(--mid-gray)',
                transition: 'all 0.2s',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 48, alignItems: 'start' }}>
            <div>
              <div style={{ display: 'inline-block', background: `${col.color}18`, border: `1px solid ${col.color}40`, borderRadius: 100, padding: '5px 14px', marginBottom: 16 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: col.color, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{col.name}</span>
              </div>
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 700, color: 'var(--navy)', marginBottom: 12, lineHeight: 1.2 }}>{col.tagline}</h2>
              <div style={{ width: 56, height: 3, background: col.color, borderRadius: 2, margin: '0 0 20px' }} />
              <p style={{ fontSize: 16, color: 'var(--mid-gray)', lineHeight: 1.7, marginBottom: 24 }}>{col.desc}</p>
              <p style={{ fontSize: 13, color: 'var(--mid-gray)', fontStyle: 'italic', marginBottom: 32 }}>Products: {col.products}</p>
              ...
              <Link to="/founding-community" className="btn btn-gold">Get First Access</Link>
            </div>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--mid-gray)', marginBottom: 16 }}>Example Messages</div>
              {col.messages.map(m => (
                <div key={m} style={{ background: 'white', borderRadius: 10, padding: '20px 24px', marginBottom: 12, boxShadow: 'var(--shadow-sm)', borderLeft: `4px solid ${col.color}` }}>
                  <p style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--navy)' }}>{m}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ background: 'var(--navy)', padding: '64px 0', textAlign: 'center' }}>
        <div className="container">
          <div className="lift-in">
            ...
            <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.65)', maxWidth: 480, margin: '0 auto 28px' }}>Join the founding community and get first access to the initial collection.</p>
