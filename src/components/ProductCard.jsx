...
export default function ProductCard({ product }) {
  ...
  const colColor = COLLECTION_COLORS[product.collectionId] || 'var(--burnt-orange)'
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', background: '#F5F0E8', aspectRatio: '4/5', overflow: 'hidden' }}>
        {hasImages ? (
          ...
          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            <div style={{ width: 80, height: 80, borderRadius: 4, background: `${colColor}18`, border: `2px dashed ${colColor}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 28, color: colColor }}>↑</span>
            </div>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: 'var(--charcoal)', textAlign: 'center', lineHeight: 1.4 }}>
              {showBack ? product.backMessage : product.frontMessage}
            </p>
            <p style={{ fontSize: 11, color: 'var(--muted-olive)', marginTop: 8, fontStyle: 'italic' }}>
      ...
      <div style={{ padding: '20px 20px 24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--charcoal)', marginBottom: 8, lineHeight: 1.3 }}>{product.title}</h3>
        <p style={{ fontSize: 13, color: 'var(--muted-olive)', lineHeight: 1.6, flex: 1, marginBottom: 16 }}>
          {showBack ? product.backMessage : product.frontMessage}
        </p>
        {cta.url.startsWith('http') ? (
          <a href={cta.url} style={{ display: 'block', textAlign: 'center', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 700, background: colColor, color: 'white', textDecoration: 'none', padding: '11px', borderRadius: 4 }}>{cta.label}</a>
        ) : (
          <Link to={cta.url} style={{ display: 'block', textAlign: 'center', fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 700, background: colColor, color: 'white', textDecoration: 'none', padding: '11px', borderRadius: 4 }}>{cta.label}</Link>
        )}
      </div>
    </div>
  )
